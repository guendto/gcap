...
=item B<Video IDs starting with a dash>

Some Youtube video IDs may start with a dash ('-'). This causes problems
with the Getopt::* modules that gcap uses to parse the command line
args, e.g.:
  % gcap -W3Cn6R0jWk
  Unknown option: W
  Unknown option: 3
...
You can work around this by using the URL form instead:
